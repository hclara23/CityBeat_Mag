...
export function StoryCard({ story, variant = "default", className }: StoryCardProps) {
  ...
  if (isFeatured) {
    return (
      <Link to={`/stories/${story.slug}`} className={cn("group block relative w-full overflow-hidden rounded-2xl aspect-[16/10]", className)}>
  ...
  return (
    <Link to={`/stories/${story.slug}`} className={cn("group flex flex-col gap-4", className)}>
